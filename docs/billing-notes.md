...
- Set the following env vars with Stripe **test** values: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_STARTER_ID`, `STRIPE_PRICE_PRO_ID`, `STRIPE_BILLING_PORTAL_RETURN_URL`.
- Run `stripe listen --forward-to localhost:3000/api/billing/webhook` to forward test events while `npm run dev` is running.
- Trigger a Starter checkout by clicking upgrade in the Plan & Billing card. Stripe will redirect back to `/settings/billing?checkout=success`.
- Confirm the webhook updates the `plan`, `stripeCustomerId`, `stripeSubscriptionId`, and `planValidUntil` columns for either `User` or `Team`.
- Cancel the subscription in the Stripe dashboard and resend the webhook to ensure we downgrade to `FREE`.

## Billing portal
- `/api/billing/portal` accepts `{ scope: 'USER' | 'TEAM', teamId? }` and returns the session URL generated by `createBillingPortalSessionForUser/Team`.
- The Plan & Billing UI triggers the TEAM scope portal so Admins can manage subscriptions without writing UI code.
- Set `STRIPE_BILLING_PORTAL_RETURN_URL` so Stripe knows where to send the user after they close the portal.

...
import { canCreateEnvelope, incrementUsageCountersOnEnvelopeCreated } from '../billing/plan-limits';
...
export const createDocumentFromTemplate = async ({
  ...
  const billingTeam = await prisma.team.findUnique({
    where: { id: teamId },
    select: {
      id: true,
      plan: true,
  ...
  const canCreateResult = await canCreateEnvelope({
    user: billingUser,
    team: billingTeam,
  });
  ...
  if (!canCreateResult.allowed) {
    throw new AppError(AppErrorCode.LIMIT_EXCEEDED, {
      message: canCreateResult.reason ?? 'You have reached your document limit.',
    });
  }
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    await incrementUsageCountersOnEnvelopeCreated({
      prismaClient: tx,
      userId,
      plan: canCreateResult.planState.plan,
    });

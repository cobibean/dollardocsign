import { OrganisationGroupType, PlanType, TeamMemberRole } from '@prisma/client';
import { match } from 'ts-pattern';

import { TEAM_MEMBER_ROLE_PERMISSIONS_MAP } from '@documenso/lib/constants/teams';
import { AppError, AppErrorCode } from '@documenso/lib/errors/app-error';
import { getMemberRoles } from '@documenso/lib/server-only/team/get-member-roles';
import { PRO_TEAM_MEMBER_LIMIT } from '@documenso/lib/universal/billing/plans';
...
export const createTeamMembers = async ({
  ...
  if (team.plan !== PlanType.PRO) {
    throw new AppError(AppErrorCode.LIMIT_EXCEEDED, {
      message: 'Inviting team members requires the Pro plan.',
    });
  }
  ...
  const existingMembers = await prisma.organisationGroupMember.findMany({
    where: {
      group: {
        teamGroups: {
          some: {
    ...
    distinct: ['organisationMemberId'],
    select: {
      organisationMemberId: true,
    },
  });

  const existingMemberIds = new Set(existingMembers.map((member) => member.organisationMemberId));

  const newUniqueMembers = membersToCreate.filter(
    (member) => !existingMemberIds.has(member.organisationMemberId),
  );

  const totalAfterAdd = existingMemberIds.size + newUniqueMembers.length;

  if (totalAfterAdd > PRO_TEAM_MEMBER_LIMIT) {
    throw new AppError(AppErrorCode.LIMIT_EXCEEDED, {
      message: `Pro plan allows up to ${PRO_TEAM_MEMBER_LIMIT} team members.`,
    });
  }
